...
const createFormState = (
  ...
  if (!price || Number(price) <= 0) {
    return _createFormState(true, 'Price is not valid. (too low)')
  }
  ...
  if (isNewPurchase) {
    if (confirmationEmail.length === 0 || email.length === 0) {
      ...
      return _createFormState(true, 'E-Mail are not matching.')
    }
  }

  if (!hasAgreedTOS) {
    return _createFormState(true, 'You need to accept terms of services.')
  }
